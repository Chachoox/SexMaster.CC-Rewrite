...
public abstract class MixinGuiPlayerTabOverlay extends Gui {
    ...
    @Shadow
    private static final Ordering<NetworkPlayerInfo> ENTRY_ORDERING = null;

    @Shadow
    abstract String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn);

    @Shadow
    abstract void drawPing(int p_175245_1_, int p_175245_2_, int p_175245_3_, NetworkPlayerInfo networkPlayerInfoIn);

    @Shadow
    abstract void drawScoreboardValues(ScoreObjective objective, int p_175247_2_, String name, int p_175247_4_, int p_175247_5_, NetworkPlayerInfo info);
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            if (k4 < list.size()) {
                ...
                if (networkplayerinfo1.getGameType() == GameType.SPECTATOR) {
                    ...
                    boolean doColor = BetterTabModule.INSTANCE.isEnabled() && FriendManager.getInstance().isFriend(s4);
                    this.mc.fontRenderer.drawStringWithShadow(s4, (float) j2, (float) k2, doColor ? ColorsModule.INSTANCE.getFriendColor().getRGB() : -1);
                }
