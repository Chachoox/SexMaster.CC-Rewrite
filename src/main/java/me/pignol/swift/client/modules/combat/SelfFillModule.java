...
public class SelfFillModule extends Module {
    ...
    private final Value<Float> height = new Value<>("Height", 5F, -5F, 5F, "Height");
    private final Value<Float> spaceCheck = new Value<>("SelfFill", 7F, 0F, 15F, "Checks for space before placing");
    private final Value<Boolean> preferEChests = new Value<>("EChests", true, "Prioritises echests instead of obsidian");
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (obbySlot == -1) {
            ChatUtil.printString(ChatFormatting.RED + "[SelfFill] No obsidian.", -554);
            setEnabled(false);
            return;
        }
